...
import { ArrowRight, CircleCheck, Minus, PackageCheck, Plus, ShieldCheck } from "lucide-react";
    ...
    <SiteShell>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24 w-full flex flex-col lg:flex-row gap-12 lg:gap-24 font-sans items-start">
        <div className="flex-1 w-full">
          ...
          <article className="flex gap-6 py-6 border-b border-line">
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div className="flex justify-between items-start gap-4">
                <div>
                  <small className="text-muted text-xs font-bold uppercase tracking-wider block mb-1">Lighting</small>
                  <h2 className="text-xl font-medium text-ink">Sol Focus Lamp</h2>
                  <p className="text-muted mt-1">Warm sand</p>
                </div>
                <div className="text-right">
                  <strong className="block text-lg font-medium text-ink">₹{(3499 * quantity).toLocaleString("en-IN")}</strong>
                  <button className="text-xs font-bold uppercase tracking-wider text-muted hover:text-ink transition-colors mt-2">Remove</button>
                </div>
              </div>
              <div className="flex items-center gap-4 bg-paper border border-line rounded-full w-fit px-1 py-1 mt-4">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-line/50 text-ink transition-colors"><Minus className="w-4 h-4" /></button>
                <span className="w-6 text-center font-medium text-sm text-ink">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} aria-label="Increase quantity" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-line/50 text-ink transition-colors"><Plus className="w-4 h-4" /></button>
              </div>
            </div>
          </article>
        </div>
        ...
        <aside className="w-full lg:w-96 shrink-0 bg-white border border-line rounded-3xl p-8 sticky top-32 shadow-sm">
          ...
          <div className="flex flex-col gap-4 text-ink mb-6">
            <div className="flex justify-between">
              ...
              <strong className="font-medium">₹{(3499 * quantity).toLocaleString("en-IN")}</strong>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between items-end">
              ...
              <strong className="text-2xl font-medium">₹{(3499 * quantity).toLocaleString("en-IN")}</strong>
            </div>
          </div>
