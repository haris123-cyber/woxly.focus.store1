  ...
  return (
    <article className="group flex flex-col font-sans">
      <div className="relative aspect-[4/5] bg-line/20 rounded-2xl overflow-hidden mb-4">
        <Link href="/product/sol-focus-lamp" aria-label={`View ${product.name}`} className="absolute inset-0">
          <Image 
            src={product.image} 
            alt={product.name} 
            fill 
      ...
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <h3 className="text-ink font-medium text-lg leading-tight">
            <Link href="/product/sol-focus-lamp" className="hover:text-muted transition-colors">
              {product.name}
            </Link>
          </h3>
        </div>
